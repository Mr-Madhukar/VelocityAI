...
export async function createCheckoutSubscription(plan: PaidPlan) {
  try {
    ...
    console.error("createCheckoutSubscription failed:", err);
    const errorObj = err as { error?: { description?: string }; message?: string };
    const detail =
      errorObj?.error?.description ||
      errorObj?.message ||
      "Payment gateway error. Please verify your Razorpay credentials.";
    return {
      ok: false as const,
      error: `Razorpay: ${detail}`,
    };
  }
}
...
export async function cancelCheckoutSubscription() {
  try {
    ...
    console.error("cancelCheckoutSubscription failed:", err);
    const errorObj = err as { error?: { description?: string }; message?: string };
    const detail = errorObj?.error?.description || errorObj?.message || "Failed to cancel subscription.";
    return {
      ok: false as const,
      error: `Razorpay: ${detail}`,
    };
