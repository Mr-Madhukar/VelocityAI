...
function RepoIndexStatusBar({
  ...
  function renderIndexStatus() {
    ...
    if (status.indexed) {
      return (
        <>
          Indexed · <span className="text-foreground">{status.fileCount} files</span> · {status.stack}
        </>
      );
    }
    return "Not indexed yet — build context first.";
  }

  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border border-border bg-background/50 px-2.5 py-1.5">
      <p className="text-[11px] text-muted-foreground">{renderIndexStatus()}</p>
      <button
        type="button"
        onClick={onIndexRepo}
        disabled={indexing}
...
function PlanResultView({
  ...
  const fileLabel = plan.files.length === 1 ? "1 file proposed" : `${plan.files.length} files proposed`;
    ...
    <div className="space-y-3 border border-border bg-background/50 p-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 font-mono text-[10px] text-muted-foreground">{fileLabel}</p>
        </div>
        <button
          type="button"
          onClick={onOpenPr}
      ...
      <section>
        ...
        <ol className="space-y-1">
          {plan.plan.map((step, i) => (
            <li key={step} className="flex gap-2 text-[11px] text-foreground/90">
      ...
      {plan.notes ? (
        <section className="border-l-2 border-amber-400/40 bg-amber-400/5 px-2.5 py-1.5">
...
export function AssistantPanel({
  ...
  function openPr() {
    ...
    startOpeningPr(async () => {
      const planSteps = plan.plan.map((s, i) => `${i + 1}. ${s}`).join("\n");
      const body = `${planSteps}\n\n${plan.notes}`;
